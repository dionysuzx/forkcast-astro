...
export function calculateForkDates(
	...
	for (let i = devnetCount - 1; i >= 0; i--) {
		...
		currentDevnetEnd = subtractDays(
			currentDevnetEnd,
			durations.DEVNET_DURATION,
		);
	}

	// EIP Selection substeps (working backwards from first devnet)
	const eipSfiDeadline = subtractDays(
		devnets[0],
		durations.EIP_SELECTION_TO_DEVNET,
	);
	const eipPfiDeadline = subtractDays(
		eipSfiDeadline,
		durations.EIP_PFI_DURATION,
	);

	// Headliner Selection substeps (working backwards from EIP PFI deadline)
	const headlinerSelectionDeadline = subtractDays(
		eipPfiDeadline,
		durations.SELECTION_TO_EIP_PFI,
	);
	const headlinerProposalDeadline = subtractDays(
		headlinerSelectionDeadline,
		durations.HEADLINER_SELECTION_DURATION,
	);
...
export function generateForkProgress(
	...
	const options: ForkProgressOptions =
		typeof optionsOrIsHistorical === "boolean"
	...
	if (options.headlinerSelectionDeadlineOverride) {
		dates.headlinerSelectionDeadline =
			options.headlinerSelectionDeadlineOverride;
	} else if (options.headlinerProposalDeadlineOverride) {
		// Recalculate selection deadline based on the proposal override
		dates.headlinerSelectionDeadline = new Date(
			options.headlinerProposalDeadlineOverride,
		);
		dates.headlinerSelectionDeadline.setDate(
			dates.headlinerSelectionDeadline.getDate() +
				durations.HEADLINER_SELECTION_DURATION,
		);
	}
