...
export const networkUpgrades: NetworkUpgrade[] = [
	{
		...
		description:
			"A complete history of all Ethereum network upgrades from the early days to the present.",
	...
	},
	{
		...
		name: "The Merge",
		description:
			"Transition to Proof of Stake, replacing energy-intensive proof-of-work mining with a more sustainable consensus mechanism.",
	...
	},
	{
		...
		description:
			'Major upgrade enabling staking withdrawals, allowing validators to withdraw their staked ETH for the first time since the Beacon Chain launch. Named after the combination of "Shanghai" and "Capella".',
		tagline:
			"Enabling staking withdrawals and completing the transition to proof-of-stake.",
		status: "Live",
	...
	},
	{
		...
		name: "Dencun Upgrade",
		description:
			"Major upgrade introducing proto-danksharding (EIP-4844) for Layer 2 scaling via blob transactions.",
		tagline:
			"Proto-danksharding brings cheaper Layer 2 transactions through blob data.",
		status: "Live",
	...
	},
	{
		...
		clientTeamPerspectives: [
			...
			},
			{
				teamName: "Erigon",
				teamType: "EL",
				headlinerBlogPostUrl:
					"https://hackmd.io/@erigon/Glamsterdam_Headliners_View",
				candidateBlogPostUrl:
					"https://github.com/erigontech/erigon/wiki/Glamsterdam-PFI-stand",
			},
			{
				teamName: "Geth",
				teamType: "EL",
			...
			},
			{
				...
				candidateBlogPostUrl:
					"https://github.com/ethereum/pm/issues/1790#issuecomment-3528064777",
			},
			{
				teamName: "Lighthouse",
				teamType: "CL",
				headlinerBlogPostUrl:
					"https://blog.sigmaprime.io/glamsterdam-headliner.html",
				candidateBlogPostUrl:
					"https://blog.sigmaprime.io/glamsterdam-eip-preferences.html",
			},
			{
				teamName: "Lodestar",
				teamType: "CL",
				...
				candidateBlogPostUrl:
					"https://blog.chainsafe.io/lodestar-glamsterdam-upgrade-proposal/",
			},
			{
				teamName: "Nethermind",
				teamType: "EL",
				...
				candidateBlogPostUrl:
					"https://x.com/URozmej/status/1986040895578296825",
			},
			{
				teamName: "Nimbus",
				teamType: "CL",
				headlinerBlogPostUrl:
					"https://notes.status.im/MJFCsbS0RTaDZYMMapR1ng?view",
			...
			},
			{
				...
				teamType: "CL",
				headlinerBlogPostUrl:
					"https://hackmd.io/@tchain/prysm-glamsterdam-headliner",
				candidateBlogPostUrl:
					"https://github.com/ethereum/pm/issues/1790#issuecomment-3524246616",
			},
			{
				teamName: "Reth",
				teamType: "EL",
				headlinerBlogPostUrl:
					"https://hackmd.io/@ZPrq5kalQqSX-138YNSJUQ/H1JafRXLle",
	...
	},
	{
		...
		name: "Hegotá Upgrade",
		description:
			"Future network upgrade currently in early planning stages. Named after Heze and Bogotá.",
...
export const activeNetworkUpgrades = networkUpgrades.filter(
	(upgrade) => !upgrade.disabled,
);

export const getUpgradeById = (id: string): NetworkUpgrade | undefined =>
	networkUpgrades.find((upgrade) => upgrade.id === id);

export const getForkDisplayName = (forkName: string): string =>
	forkName === "Hegota" ? "Hegotá" : forkName;

export const getUpgradeProposalGroups = (
	forkName: string,
	proposals: Proposal[],
) => {
	const inFork = proposals.filter((proposal) =>
		proposal.forkRelationships.some(
			(fork) => fork.forkName.toLowerCase() === forkName.toLowerCase(),
		),
	);
	return {
		headliners: inFork.filter((proposal) =>
			proposal.forkRelationships.some(
				(fork) =>
		...
		candidates: inFork.filter((proposal) =>
			proposal.forkRelationships.some(
				(fork) =>
					fork.forkName.toLowerCase() === forkName.toLowerCase() &&
					fork.wasHeadlinerCandidate &&
					!fork.isHeadliner,
			),
		),
		all: inFork,
