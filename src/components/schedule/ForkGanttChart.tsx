...
import { parseShortDate } from "@/domain/schedule/fork-date-calculator";
...
function getDateFromPhase(phase: ForkProgress["phases"][0]): {
	start: Date | null;
	end: Date | null;
} {
...
const ForkGanttChart: React.FC<ForkGanttChartProps> = ({
			...
			fork.progress.phases.forEach((phase) => {
				...
				if (phase.substeps) {
					phase.substeps.forEach((substep) => {
						const substepDate = parseShortDate(
							substep.date || substep.projectedDate || "",
						);
				...
				if (phase.devnets) {
					phase.devnets.forEach((devnet) => {
						const devnetDate = parseShortDate(
							devnet.date || devnet.projectedDate || "",
						);
				...
				if (phase.testnets) {
					phase.testnets.forEach((testnet) => {
						...
						const testnetDate = parseShortDate(
							testnet.date || testnet.projectedDate || "",
						);
				...
				if (milestoneDates.length > 0) {
					phaseStart = new Date(
						Math.min(...milestoneDates.map((d) => d.getTime())),
					);
					phaseEnd = new Date(
						Math.max(...milestoneDates.map((d) => d.getTime())),
					);
				} else {
				...
				milestoneItems.forEach((milestone) => {
					if (
						milestone.startDate < timelineStart ||
						milestone.startDate > timelineEnd
					)
						return;
	...
	const percent = (value: number): string => `${value.toFixed(4)}%`;
	...
	return (
		<div className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg overflow-hidden">
			...
			<div className="overflow-x-auto">
				<div
					style={{ minWidth: `${labelWidth + months.length * 80}px` }}
					className="relative"
				>
					{/* Month header */}
					<div
					...
					<div className="relative">
						...
						{forks.map((fork, forkIdx) => {
							const forkItems = timelineItems.items.filter(
								(item) => item.forkName === fork.name,
							);
							const phaseItems = forkItems.filter(
										...
										return (
											...
											>
												...
												<div
													...
													style={{ width: months.length * 80 }}
												>
													{/* Month grid lines */}
													<div className="absolute inset-0 flex">
														{months.map((month) => (
															<div
																key={`${fork.name}-${phase.phaseName}-${month.label}`}
													...
													{phase.startDate.getTime() !==
														...
														<div
															...
															style={{
																left: percent(getPosition(phase.startDate)),
																width: percent(
																	Math.max(
																		0.5,
																		getPosition(phase.endDate) -
																			getPosition(phase.startDate),
																	),
																),
													...
													{milestones.map((milestone) => (
														<div
															key={`${milestone.label}-${milestone.startDate.toISOString()}`}
															className="absolute top-1/2 -translate-y-1/2 group hover:z-20"
															style={{
																left: percent(getPosition(milestone.startDate)),
															}}
														>
