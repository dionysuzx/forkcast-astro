...
const EditableDateCell: React.FC<EditableDateCellProps> = ({
	...
	if (!isEditable) {
		if (!calculatedDate) {
			return (
				<span className="text-slate-400 dark:text-slate-400 text-sm italic">
					N/A
				</span>
			);
		}
		return (
			<div className="flex items-center gap-1">
				...
				<div
					className={`text-slate-700 dark:text-slate-300 text-sm ${dateWidth}`}
				>
					{displayDate}
				</div>
				<span
					className={`text-xs ${gapWidth} text-right ${gapIsNegative ? "text-red-600 dark:text-red-400 font-semibold" : gapIsWarning ? "text-amber-600 dark:text-amber-400 font-medium" : "text-slate-400 dark:text-slate-400"}`}
				>
					{gapText || ""}
				</span>
	...
	if (isSourceLocked) {
		return (
			<div className="flex items-center gap-1 group">
				<div
					className={`inline-flex items-center justify-center w-4 py-0.5 rounded text-xs font-medium ${isOverdue ? "bg-red-100 text-red-800 dark:bg-red-900/20 dark:text-red-300" : "bg-slate-100 text-slate-600 dark:bg-slate-700 dark:text-slate-300"}`}
				>
					{isOverdue ? "!" : "○"}
				</div>
				<div
					className={`text-sm ${dateWidth} ${isOverdue ? "text-red-600 dark:text-red-400 font-semibold" : "text-slate-700 dark:text-slate-300"}`}
				>
					{displayDate}
				</div>
				<span
				...
				<span
					className={`${iconWidth} text-xs text-purple-500 text-center`}
					title="Date from meta thread"
				>
					🔒
				</span>
			</div>
	...
	return (
		<div className="flex items-center gap-1 group">
			...
			<button
				...
				title={
					isLocked ? "Unlock (recalculate from mainnet date)" : "Lock this date"
				}
			>
				{isLocked ? "🔒" : "🔓"}
			</button>
