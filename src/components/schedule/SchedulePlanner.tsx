...
type MobileFork = ScheduledFork;
const DEFAULT_STATE = defaultPlanningState;
...
const loadMobileNoticeDismissed = (): boolean => {
...
> = {
	...
	SEPOLIA_TO_HOODI: {
		label: "Sepolia → Hoodi",
		description: "Days between Sepolia and Hoodi testnets",
	},
	DEVNET_TO_SEPOLIA: {
		label: "Last Devnet → Sepolia",
		description: "Days between last devnet and Sepolia",
	},
	DEVNET_DURATION: {
		label: "Devnet Duration",
		description: "Days between each devnet",
	},
	DEVNET_COUNT: {
		label: "Default Devnet Count",
		description: "Default number of devnets (can override per fork)",
	},
	EIP_SELECTION_TO_DEVNET: {
		label: "CFI → First Devnet",
		description: "Days between CFI deadline and first devnet",
	},
	EIP_PFI_DURATION: {
		label: "PFI → CFI",
		description: "Expected days for EIP proposals (PFI to CFI)",
	},
	HEADLINER_SELECTION_DURATION: {
		label: "Proposal → Selection",
		description: "Expected days for headliner review/selection",
	},
	SELECTION_TO_EIP_PFI: {
		label: "Selection → PFI",
		description:
			"Expected days between headliner selection and EIP proposal window",
	},
};

const SchedulePage: React.FC = () => {
	const [glamsterdamMainnetDate, setGlamsterdamMainnetDate] = useState<string>(
		DEFAULT_STATE.glamsterdamMainnetDate,
	);
	const [hegotaMainnetDate, setHegotaMainnetDate] = useState<string>(
		DEFAULT_STATE.hegotaMainnetDate,
	);
	const [glamsterdamDevnetCount, setGlamsterdamDevnetCount] = useState<number>(
		DEFAULT_STATE.glamsterdamDevnetCount,
	);
	const [hegotaDevnetCount, setHegotaDevnetCount] = useState<number>(
		DEFAULT_STATE.hegotaDevnetCount,
	);
	const [lockedDates, setLockedDates] = useState<Record<string, string>>(
		DEFAULT_STATE.lockedDates,
	);
	...
	const planningState = useMemo<PlanningTableState>(
		() => ({
	...
	const schedulePlan = useMemo(
		() => buildSchedulePlan(planningState),
		[planningState],
	);
	const {
		fusaka: fusakaPlan,
		glamsterdam: glamsterdamPlan,
		hegota: hegotaPlan,
	} = schedulePlan.forksById;
	const fusakaProgress = fusakaPlan.progress;
	const dynamicGlamsterdamProjection = glamsterdamPlan.progress;
	const dynamicHegotaProjection = hegotaPlan.progress;
	...
	useEffect(() => {
		...
		setGlamsterdamMainnetDate(savedState.glamsterdamMainnetDate);
		setHegotaMainnetDate(savedState.hegotaMainnetDate);
		setGlamsterdamDevnetCount(savedState.glamsterdamDevnetCount);
		setHegotaDevnetCount(savedState.hegotaDevnetCount);
		setLockedDates(savedState.lockedDates);
		setPhaseDurations(savedState.phaseDurations);
		setMobileNoticeDismissed(loadMobileNoticeDismissed());
	...
	}, [loadedSavedState, planningState]);
	...
	): { days: number; isUnderExpected: boolean } | null => {
	...
	const getMilestoneOrder = (
		fork: string,
	): Array<{ phaseId: string; itemName: string }> => {
		const projection =
			fork === "glamsterdam"
				? dynamicGlamsterdamProjection
				: dynamicHegotaProjection;
		const devnetCount =
			fork === "glamsterdam" ? glamsterdamDevnetCount : hegotaDevnetCount;
		...
		const testnetPhase = projection.phases.find(
			(p) => p.phaseId === "public-testnets",
		);
	...
	const getCalculatedDateForMilestone = (
		fork: string,
		phaseId: string,
		itemName: string,
	): string => {
	...
	const lockDate = (
		...
		const targetIndex = milestones.findIndex(
			(m) => m.phaseId === phaseId && m.itemName === itemName,
		);
		...
		setLockedDates((prev) => {
			...
			for (let i = 0; i <= targetIndex; i++) {
				...
				if (!(key in newLocked)) {
					const calcDate = getCalculatedDateForMilestone(
						fork,
						milestone.phaseId,
						milestone.itemName,
					);
			...
			<div className="max-w-7xl mx-auto">
				...
				<div className="mb-6">
					...
					<p className="text-sm text-slate-500 dark:text-slate-400">
						Internal planning tool for ACD facilitators, client teams, and
						testers. Dates shown are hypothetical projections, not commitments.
					</p>
					<p className="mt-1 text-xs text-slate-500 dark:text-slate-400">
						Edit and lock dates to explore planning scenarios. Changes are saved
					...
					<div className="md:hidden mb-4 p-3 bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-lg flex items-start gap-3">
						<div className="flex-1">
							<p className="text-sm text-amber-800 dark:text-amber-200">
								This planning tool is optimized for desktop viewing. For the
								best experience, use a larger screen.
							</p>
						</div>
						<button
							type="button"
				...
				<div className="md:hidden mb-4 flex items-center gap-2">
					...
					<div className="flex rounded-lg border border-slate-300 dark:border-slate-600 overflow-hidden">
						{(["fusaka", "glamsterdam", "hegota"] as MobileFork[]).map(
							(fork) => (
								<button
									type="button"
									key={fork}
				...
				{(() => {
					...
					const calculateGap = (
						...
						forkKey: "fusaka" | "glamsterdam" | "hegota",
					...
					const getMonthsDaysBetween = (
						...
						if (to.getDate() < from.getDate()) {
							...
							const days = Math.floor(
								(to.getTime() - prevMonth.getTime()) / (1000 * 60 * 60 * 24),
							);
							return { months, days };
						}
					...
					const pectraMainnet =
						parseShortDate("May 7, 2025") ?? new Date(2025, 4, 7);
					const fusakaMainnet =
						parseShortDate("Dec 3, 2025") ?? new Date(2025, 11, 3);
									...
									<div className="flex items-start justify-between">
										<div>
											...
											<p className="text-sm text-slate-600 dark:text-slate-300">
												See the cascading impacts of scheduling decisions. Click
												dates to adjust. Changes are saved automatically.
											</p>
										</div>
									...
									<div className="px-4 py-3 border-b border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-700/30">
										...
										<div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-[10px]">
											...
											<div className="space-y-1">
												<div className="font-medium text-slate-700 dark:text-slate-300 border-b border-slate-200 dark:border-slate-600 pb-1">
													Headliners
												</div>
												<div className="flex justify-between items-center">
													<span
														...
														title={
															DURATION_LABELS.HEADLINER_SELECTION_DURATION
																.description
														}
													>
														Proposal → Selection
													...
													<input
														type="number"
														min="1"
														value={phaseDurations.HEADLINER_SELECTION_DURATION}
														onChange={(e) =>
															updateDuration(
																"HEADLINER_SELECTION_DURATION",
														...
														className="px-1 py-0.5 border border-slate-300 dark:border-slate-600 rounded bg-white dark:bg-slate-700 text-slate-900 dark:text-slate-100 focus:outline-none focus:ring-1 focus:ring-purple-500 w-10 text-center text-[10px]"
													/>
												</div>
												<div className="flex justify-between items-center">
													<span
														className="text-slate-600 dark:text-slate-400"
														title={
															DURATION_LABELS.SELECTION_TO_EIP_PFI.description
														}
													>
														Selection → PFI
													</span>
													<input
														type="number"
														min="1"
														value={phaseDurations.SELECTION_TO_EIP_PFI}
														onChange={(e) =>
															updateDuration(
																"SELECTION_TO_EIP_PFI",
																Number.parseInt(e.target.value, 10) || 1,
											...
											<div className="space-y-1">
												...
												<div className="flex justify-between items-center">
													<span
														className="text-slate-600 dark:text-slate-400"
														title={DURATION_LABELS.EIP_PFI_DURATION.description}
													>
														PFI → CFI
													</span>
													<input
														type="number"
														min="1"
														value={phaseDurations.EIP_PFI_DURATION}
														onChange={(e) =>
															updateDuration(
																"EIP_PFI_DURATION",
																Number.parseInt(e.target.value, 10) || 1,
												...
												<div className="flex justify-between items-center">
													<span
														className="text-slate-600 dark:text-slate-400"
														title={
															DURATION_LABELS.EIP_SELECTION_TO_DEVNET
																.description
														}
													>
														CFI → Devnet
													...
													<input
														type="number"
														min="1"
														value={phaseDurations.EIP_SELECTION_TO_DEVNET}
														onChange={(e) =>
															updateDuration(
																"EIP_SELECTION_TO_DEVNET",
																Number.parseInt(e.target.value, 10) || 1,
											...
											<div className="space-y-1">
												...
												<div className="flex justify-between items-center">
													<span
														className="text-slate-600 dark:text-slate-400"
														title={DURATION_LABELS.DEVNET_DURATION.description}
													>
														Between Each
													</span>
													<input
														type="number"
														min="1"
														value={phaseDurations.DEVNET_DURATION}
														onChange={(e) =>
															updateDuration(
																"DEVNET_DURATION",
																Number.parseInt(e.target.value, 10) || 1,
												...
												<div className="flex justify-between items-center">
													<span
														className="text-slate-600 dark:text-slate-400"
														title={
															DURATION_LABELS.DEVNET_TO_SEPOLIA.description
														}
													>
														Last → Sepolia
													</span>
													<input
														type="number"
														min="1"
														value={phaseDurations.DEVNET_TO_SEPOLIA}
														onChange={(e) =>
															updateDuration(
																"DEVNET_TO_SEPOLIA",
																Number.parseInt(e.target.value, 10) || 1,
											...
											<div className="space-y-1">
												...
												<div className="flex justify-between items-center">
													<span
														className="text-slate-600 dark:text-slate-400"
														title={DURATION_LABELS.SEPOLIA_TO_HOODI.description}
													>
														Sepolia → Hoodi
													</span>
													<input
														type="number"
														min="1"
														value={phaseDurations.SEPOLIA_TO_HOODI}
														onChange={(e) =>
															updateDuration(
																"SEPOLIA_TO_HOODI",
																Number.parseInt(e.target.value, 10) || 1,
												...
												<div className="flex justify-between items-center">
													<span
														className="text-slate-600 dark:text-slate-400"
														title={DURATION_LABELS.HOODI_TO_MAINNET.description}
													>
														Hoodi → Mainnet
													</span>
													<input
														type="number"
														min="1"
														value={phaseDurations.HOODI_TO_MAINNET}
														onChange={(e) =>
															updateDuration(
																"HOODI_TO_MAINNET",
																Number.parseInt(e.target.value, 10) || 1,
									...
									<table className="w-full text-sm">
										<thead className="bg-slate-50 dark:bg-slate-700/50">
											<tr>
												...
												<th
													className={`px-3 py-2 text-left text-xs font-semibold text-slate-600 dark:text-slate-300 uppercase tracking-wider ${mobileFork === "fusaka" ? "" : "hidden"} md:table-cell`}
												>
													Fusaka
												</th>
												<th
													className={`px-3 py-2 text-left text-xs font-semibold text-slate-600 dark:text-slate-300 uppercase tracking-wider ${mobileFork === "glamsterdam" ? "" : "hidden"} md:table-cell`}
												>
													<a
												...
												<th
													className={`px-3 py-2 text-left text-xs font-semibold text-slate-600 dark:text-slate-300 uppercase tracking-wider ${mobileFork === "hegota" ? "" : "hidden"} md:table-cell`}
												>
													<a
										...
										<tbody className="divide-y divide-slate-200 dark:divide-slate-700">
											{schedulePlan.phases
												...
												.map((phase) => {
													const fusakaPhase = fusakaProgress.phases.find(
														(p) => p.phaseId === phase.id,
													);
													const glamsterdamPhase =
														dynamicGlamsterdamProjection.phases.find(
															(p) => p.phaseId === phase.id,
														);
													const hegotaPhase =
														dynamicHegotaProjection.phases.find(
															(p) => p.phaseId === phase.id,
														);
														...
														<React.Fragment key={phase.id}>
															<tr
																className={
																	...
																	phase.id === "headliner-selection" ||
																	phase.id === "eip-selection"
																		? "bg-slate-100 dark:bg-slate-700/50"
																		: "hover:bg-slate-50 dark:hover:bg-slate-700/30"
																}
															>
																<td
																	className={`sticky left-0 ${phase.id === "development" || phase.id === "headliner-selection" || phase.id === "eip-selection" ? "bg-slate-100 dark:bg-slate-700/50" : "bg-white dark:bg-slate-800"} px-3 py-1.5 font-medium text-slate-900 dark:text-slate-100 border-r border-slate-200 dark:border-slate-600`}
																>
																	{phase.title}
																</td>
																<td
																	className={`px-3 py-1.5 ${mobileFork === "fusaka" ? "" : "hidden"} md:table-cell`}
																>
																	{fusakaPhase &&
																		phase.id !== "development" &&
																		phase.id !== "headliner-selection" &&
																		phase.id !== "eip-selection" && (
																			<div className="flex items-center gap-2">
																				<div
																					className={`inline-flex items-center justify-center w-4 py-0.5 rounded text-xs font-medium ${
																						fusakaPhase.status === "completed"
																				...
																				<div className="text-slate-700 dark:text-slate-300 text-sm">
																					{fusakaPhase.actualEndDate
																						? fusakaPhase.actualEndDate
																						: fusakaPhase.projectedDate ||
																							fusakaPhase.actualStartDate}
																				</div>
																			</div>
																		)}
																	{phase.id === "development" && (
																...
																<td
																	className={`px-3 py-1.5 ${mobileFork === "glamsterdam" ? "" : "hidden"} md:table-cell`}
																>
																	{glamsterdamPhase &&
																		phase.id !== "development" &&
																		phase.id !== "headliner-selection" &&
																		phase.id !== "eip-selection" && (
																			<div className="flex items-center gap-2">
																				...
																				<div className="text-slate-700 dark:text-slate-300 text-sm">
																					{glamsterdamPhase.actualEndDate ||
																						glamsterdamPhase.projectedDate ||
																						glamsterdamPhase.actualStartDate}
																				</div>
																			</div>
																		)}
																	{phase.id === "development" && (
																		<div className="flex items-center gap-2">
																			<span className="text-slate-500 dark:text-slate-400 text-sm">
																				{glamsterdamDevnetCount} devnets
																			</span>
																			<div className="flex items-center">
																				<button
																					type="button"
																					onClick={() =>
																						setGlamsterdamDevnetCount(
																							Math.max(
																								1,
																								glamsterdamDevnetCount - 1,
																							),
																						)
																					}
																...
																<td
																	className={`px-3 py-1.5 ${mobileFork === "hegota" ? "" : "hidden"} md:table-cell`}
																>
																	{hegotaPhase &&
																		phase.id !== "development" &&
																		phase.id !== "headliner-selection" &&
																	...
																	{phase.id === "development" && (
																		<div className="flex items-center gap-2">
																			<span className="text-slate-500 dark:text-slate-400 text-sm">
																				{hegotaDevnetCount} devnets
																			</span>
																			<div className="flex items-center">
																				<button
																					type="button"
																					onClick={() =>
																						setHegotaDevnetCount(
																							Math.max(
																								1,
																								hegotaDevnetCount - 1,
																							),
															...
															{(phase.id === "headliner-selection" ||
																phase.id === "eip-selection") &&
																(glamsterdamPhase?.substeps ||
																	hegotaPhase?.substeps ||
																	fusakaPhase?.substeps) &&
																(
																	glamsterdamPhase?.substeps ||
																	hegotaPhase?.substeps ||
																	fusakaPhase?.substeps
																	...
																	const fusakaSubstep =
																		fusakaPhase?.substeps?.[idx];
																	const glamsterdamSubstep =
																		glamsterdamPhase?.substeps?.[idx];
																	const hegotaSubstep =
																		hegotaPhase?.substeps?.[idx];
																	const fusakaGap = fusakaSubstep
																		? calculateGap(
																				fusakaSubstep.date ||
																					fusakaSubstep.projectedDate,
																				"fusaka",
																			)
																		: { text: "", isNegative: false };
																	const glamsterdamCalcDate =
																		glamsterdamSubstep?.date ||
																		glamsterdamSubstep?.projectedDate ||
																		"";
																	const glamsterdamEffectiveDate =
																		getEffectiveDate(
																			"glamsterdam",
																			phase.id,
																			substep.name,
																	...
																	const glamsterdamGap = glamsterdamSubstep
																		? calculateGap(
																				glamsterdamEffectiveDate,
																				"glamsterdam",
																			)
																		: { text: "", isNegative: false };
																	const hegotaCalcDate =
																		hegotaSubstep?.projectedDate || "";
																	const hegotaEffectiveDate = getEffectiveDate(
																		"hegota",
																		phase.id,
																		substep.name,
																		hegotaCalcDate,
																	);
																	const hegotaGap = hegotaSubstep
																		? calculateGap(
																				hegotaEffectiveDate,
																				"hegota",
																			)
																	...
																	const getDurationConfig = (
																		...
																		if (name === "Selection Date")
																			return {
																				prevPhase: "headliner-selection",
																				prevItem: "Proposal Deadline",
																				expected:
																					phaseDurations.HEADLINER_SELECTION_DURATION,
																			};
																		if (name === "PFI Deadline")
																			return {
																				prevPhase: "headliner-selection",
																				prevItem: "Selection Date",
																				expected:
																					phaseDurations.SELECTION_TO_EIP_PFI,
																			};
																		if (name === "CFI Deadline")
																			return {
																				prevPhase: "eip-selection",
																				prevItem: "PFI Deadline",
																				expected:
																					phaseDurations.EIP_PFI_DURATION,
																			};
																		return null;
																	};
																	...
																	const getPhaseProgress = (fork: string) => {
																		if (fork === "fusaka")
																			return fusakaProgress;
																		if (fork === "glamsterdam")
																			return dynamicGlamsterdamProjection;
																		return dynamicHegotaProjection;
																	};
																	const calcDurationWarning = (
																		fork: string,
																		currentDate: string,
																	) => {
																		if (!durationConfig || !currentDate)
																			return null;
																		const progress = getPhaseProgress(fork);
																		const prevPhaseData = progress.phases.find(
																			(p) =>
																				p.phaseId === durationConfig.prevPhase,
																		);
																		const prevSubstep =
																			prevPhaseData?.substeps?.find(
																				(s) =>
																					s.name === durationConfig.prevItem,
																			);
																		const prevDate =
																			prevSubstep?.date ||
																			prevSubstep?.projectedDate ||
																			"";
																		const prevEffectiveDate = getEffectiveDate(
																			fork,
																			durationConfig.prevPhase,
																			durationConfig.prevItem,
																			prevDate,
																		);
																		return getDurationWarning(
																			prevEffectiveDate,
																			currentDate,
																			durationConfig.expected,
																		);
																	};

																	const glamsterdamDuration =
																		calcDurationWarning(
																			"glamsterdam",
																			glamsterdamEffectiveDate,
																		);
																	const hegotaDuration = calcDurationWarning(
																		"hegota",
																		hegotaEffectiveDate,
																	);
																	...
																	return (
																		<tr
																			key={`${phase.id}-substep-${substep.name}`}
																			className="hover:bg-slate-50 dark:hover:bg-slate-700/30 bg-slate-50/50 dark:bg-slate-800/50"
																		>
																			...
																			>
																				{glamsterdamSubstep ? (
																					<EditableDateCell
																						...
																						gapText={
																							glamsterdamDuration
																								? `(${glamsterdamDuration.days >= 0 ? "+" : ""}${glamsterdamDuration.days}d)`
																								: glamsterdamGap.text
																						}
																						gapIsNegative={
																							glamsterdamDuration
																								? glamsterdamDuration.days < 0
																								: glamsterdamGap.isNegative
																						}
																						gapIsWarning={
																							glamsterdamDuration?.isUnderExpected &&
																							glamsterdamDuration.days >= 0
																						}
																					/>
																				) : null}
																			...
																			>
																				{hegotaSubstep ? (
																					<EditableDateCell
																						...
																						gapText={
																							hegotaDuration
																								? `(${hegotaDuration.days >= 0 ? "+" : ""}${hegotaDuration.days}d)`
																								: hegotaGap.text
																						}
																						gapIsNegative={
																							hegotaDuration
																								? hegotaDuration.days < 0
																								: hegotaGap.isNegative
																						}
																						gapIsWarning={
																							hegotaDuration?.isUnderExpected &&
																							hegotaDuration.days >= 0
																						}
																						isSourceLocked={
																							phase.id ===
															...
															{phase.id === "development" &&
																(() => {
																	const fusakaDevnetCount =
																		fusakaPhase?.devnets?.length || 0;
																	const glamDevnetCount =
																		glamsterdamPhase?.devnets?.length || 0;
																	const hegotaDevnetCount =
																		hegotaPhase?.devnets?.length || 0;
																	const maxDevnets = Math.max(
																		fusakaDevnetCount,
																		glamDevnetCount,
																		hegotaDevnetCount,
																	);
																	...
																	return Array.from(
																		...
																		(_, idx) => {
																			const fusakaDevnet =
																				fusakaPhase?.devnets?.[idx];
																			const glamDevnet =
																				glamsterdamPhase?.devnets?.[idx];
																			const hegotaDevnet =
																				hegotaPhase?.devnets?.[idx];
																			const devnetName = `Devnet-${idx}`;

																			return (
																				<tr
																					key={`${phase.id}-${devnetName}`}
																					className="hover:bg-slate-50 dark:hover:bg-slate-700/30 bg-slate-50/50 dark:bg-slate-800/50"
																				>
																					<td className="sticky left-0 bg-slate-50/50 dark:bg-slate-800/50 px-3 py-1.5 text-slate-700 dark:text-slate-300 border-r border-slate-200 dark:border-slate-600 pl-8 text-sm">
																						{devnetName}
																					...
																					>
																						{fusakaDevnet ? (
																							(() => {
																								const fusakaDevnetGap =
																									calculateGap(
																										fusakaDevnet.date ||
																											fusakaDevnet.projectedDate,
																										"fusaka",
																									);
																								return (
																									<EditableDateCell
																										...
																										onUnlock={unlockDate}
																										gapText={
																											fusakaDevnetGap.text
																										}
																										gapIsNegative={
																											fusakaDevnetGap.isNegative
																										}
																									/>
																								);
																							})()
																					...
																					>
																						{glamDevnet ? (
																							(() => {
																								const glamDevnetDate =
																									glamDevnet.date ||
																									glamDevnet.projectedDate ||
																									"";
																								const effectiveGlamDevnetDate =
																									getEffectiveDate(
																										"glamsterdam",
																										"development",
																										devnetName,
																					...
																					>
																						{hegotaDevnet ? (
																							(() => {
																								const hegotaDevnetDate =
																									hegotaDevnet.date ||
																									hegotaDevnet.projectedDate ||
																									"";
																								const effectiveHegotaDevnetDate =
																									getEffectiveDate(
																										"hegota",
																										"development",
																										devnetName,
																								...
																								const hegotaDevnetGap =
																									calculateGap(
																										effectiveHegotaDevnetDate,
																										"hegota",
																									);
																								return (
																									<EditableDateCell
																										...
																										onUnlock={unlockDate}
																										gapText={
																											hegotaDevnetGap.text
																										}
																										gapIsNegative={
																											hegotaDevnetGap.isNegative
																										}
																									/>
																								);
																							})()
											...
											{/* Testnet detail rows */}
											{fusakaProgress.phases
												.find((p) => p.phaseId === "public-testnets")
												?.testnets?.map((testnet, idx) => {
													const fusakaTestnetPhase = fusakaProgress.phases.find(
														(p) => p.phaseId === "public-testnets",
													);
													const glamsterdamTestnetPhase =
														dynamicGlamsterdamProjection.phases.find(
															(p) => p.phaseId === "public-testnets",
														);
													const hegotaTestnetPhase =
														dynamicHegotaProjection.phases.find(
															(p) => p.phaseId === "public-testnets",
														);
														...
														>
															...
															>
																{fusakaTestnetPhase?.testnets?.[idx] &&
																	(() => {
																		const fusakaTestnet =
																			fusakaTestnetPhase.testnets[idx];
																		const fusakaTestnetGap = calculateGap(
																			fusakaTestnet.date ||
																				fusakaTestnet.projectedDate,
																			"fusaka",
																		);
																		return (
																			<EditableDateCell
																				...
																				isCompleted={
																					fusakaTestnet.status === "completed"
																				}
																				isEditable={false}
																				lockedDates={lockedDates}
																				onLock={lockDate}
															...
															>
																{glamsterdamTestnetPhase?.testnets?.[idx] &&
																	(glamsterdamTestnetPhase.testnets[idx]
																		.status === "deprecated" ? (
																		<div className="text-slate-400 dark:text-slate-400 text-sm italic">
																			Deprecated
																		</div>
																	) : (
																		(() => {
																			const glamTestnet =
																				glamsterdamTestnetPhase.testnets[idx];
																			const glamTestnetDate =
																				glamTestnet.date ||
																				glamTestnet.projectedDate ||
																				"";
																			const effectiveGlamTestnetDate =
																				getEffectiveDate(
																					"glamsterdam",
																					"public-testnets",
																					testnet.name,
																			...
																			const glamTestnetGap = calculateGap(
																				effectiveGlamTestnetDate,
																				"glamsterdam",
																			);
																			return (
																				<EditableDateCell
															...
															>
																{hegotaTestnetPhase?.testnets?.[idx] &&
																	(hegotaTestnetPhase.testnets[idx].status ===
																	"deprecated" ? (
																		<div className="text-slate-400 dark:text-slate-400 text-sm italic">
																			Deprecated
																		</div>
																		...
																		(() => {
																			const hegotaTestnet =
																				hegotaTestnetPhase.testnets[idx];
																			const hegotaTestnetDate =
																				hegotaTestnet.date ||
																				hegotaTestnet.projectedDate ||
																				"";
																			const effectiveHegotaTestnetDate =
																				getEffectiveDate(
																					"hegota",
																					"public-testnets",
																					testnet.name,
																			...
																			const hegotaTestnetGap = calculateGap(
																				effectiveHegotaTestnetDate,
																				"hegota",
																			);
																			return (
																				<EditableDateCell
																					...
																					isCompleted={
																						hegotaTestnet.status === "completed"
																					}
																					isEditable={true}
																					lockedDates={lockedDates}
																					onLock={lockDate}
											...
											<tr className="bg-slate-50 dark:bg-slate-700/50 font-semibold">
												...
												<td
													className={`px-3 py-1.5 text-slate-900 dark:text-slate-100 text-sm ${mobileFork === "fusaka" ? "" : "hidden"} md:table-cell`}
												>
													<div className="flex flex-col">
														...
														{(() => {
															...
															return (
																gap.text && (
																	<span
																		className={`text-xs ${gap.isNegative ? "text-red-600 dark:text-red-400 font-semibold" : "text-slate-400 dark:text-slate-400"}`}
																	>
																		{gap.text}
																	</span>
																)
														...
														<span className="text-xs text-purple-600 dark:text-purple-400 font-medium">
															{formatMonthsDays(pectraMainnet, fusakaMainnet)}{" "}
															after Pectra
														</span>
													</div>
												</td>
												...
												>
													<div className="flex flex-col gap-1">
														<input
															...
															onChange={(e) =>
																setGlamsterdamMainnetDate(e.target.value)
															}
														...
														/>
														{(() => {
															const glamDate = parseLocalDate(
																glamsterdamMainnetDate,
															);
															const dateStr = glamDate.toLocaleDateString(
																"en-US",
																{
																	month: "short",
																	day: "numeric",
												...
												>
													<div className="flex flex-col gap-1">
														<input
															...
															onChange={(e) =>
																setHegotaMainnetDate(e.target.value)
															}
															className="px-1.5 py-0.5 text-sm border border-slate-300 dark:border-slate-600 rounded bg-white dark:bg-slate-700 text-slate-900 dark:text-slate-100 focus:outline-none focus:ring-1 focus:ring-purple-500 cursor-pointer"
														...
														{(() => {
															const hegotaDate =
																parseLocalDate(hegotaMainnetDate);
															const dateStr = hegotaDate.toLocaleDateString(
																"en-US",
																{
																	month: "short",
																	day: "numeric",
				...
				<div className="mb-10">
					<ForkGanttChart
						forks={[
							{ name: "Fusaka", progress: fusakaProgress, color: "#10b981" },
							{
								name: "Glamsterdam",
								progress: dynamicGlamsterdamProjection,
								color: "#6366f1",
							},
							{
								name: "Hegota",
								progress: dynamicHegotaProjection,
								color: "#f59e0b",
							},
						]}
						...
						monthsToShow={(() => {
							...
							const months =
								(hegotaDate.getFullYear() - start.getFullYear()) * 12 +
								(hegotaDate.getMonth() - start.getMonth()) +
								2; // +2 for buffer
